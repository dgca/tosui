---
title: Customization
description: "Override Tosui CSS variables for colors, spacing, typography, borders, and shadows."
---

# Customization

Tosui exposes its design tokens as CSS custom properties. Override a token after importing the library styles to change every component that uses it.
...
Tosui maps these primitive values to the active light or dark theme.
...
2. **Semantic variables** (`--t-color-*`) reference the matching primitive for the active theme
...
Use primitive variables to define a theme. Use semantic variables for a scoped override in the active theme.
...
Scoped overrides use semantic variables (`--t-color-*`) because they apply to the active theme only.
...
Without an explicit value, Tosui follows the system color scheme. Set `data-theme` to force a mode.

## Fonts

Tosui uses system fonts by default. To use custom fonts:

### Use Tosui's IBM Plex fonts
