---
title: Grid
description: "Reference for responsive Grid columns, rows, gaps, track alignment, and inherited Box props."
---
...
`Grid` renders a polymorphic CSS Grid container. Its convenience props map to grid templates, gaps, item alignment, and track alignment.
...
`columns` and `rows` accept responsive objects.
...
| justify | `ResponsiveValue<"start" \| "center" \| "end" \| "space-between" \| "space-around" \| "space-evenly">` | - | Distribute tracks horizontally; takes precedence over `justifyContent` |
